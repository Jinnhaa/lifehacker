...
interface DecisionRow { id: string; status: string }
...
const json = (value: Readonly<Record<string, unknown>>): JSONValue => value as JSONValue;

export class SupabaseDecisionLearningRepository implements DecisionLearningRepository {
  constructor(private readonly sql: Sql) {}

  async recordMaterialDecision(input: MaterialDecisionInput, userReason: string | null): Promise<MaterialDecisionRecord> {
    return this.sql.begin(async (tx) => {
      ...
      `;
      if (existing[0]) {
        const feedback = await tx<FeedbackRow[]>`
  ...
  async recordPendingReason(userId: UserId, reason: string, messageId: string, now: Date): Promise<boolean> {
    return this.sql.begin(async (tx) => {
      const decisions = await tx<DecisionRow[]>`
  ...
  async createLearningCasesForDay(input: DecisionOutcomeInput): Promise<number> {
    ...
    return this.sql.begin(async (tx) => {
      ...
      const decisions = await tx<{
        id: string; feedback_id: string; user_choice: unknown; user_reason: string | null;
        ai_recommendation: unknown; impact: unknown;
      }[]>`
      ...
      for (const decision of decisions) {
        const evidence = await tx<{ id: string; event_type: string; aggregate_type: string; aggregate_id: string }[]>`
        ...
        const impact = decision.impact && typeof decision.impact === "object" ? decision.impact as Record<string, unknown> : {};
        const cases = await tx<{ id: string }[]>`
          ...
          }))},${tx.json((decision.ai_recommendation ?? {}) as JSONValue)},${decision.id},${decision.feedback_id},'closed',${input.observedAt})
          returning id
        `;
        ...
        await tx`
          ...
          values(${input.userId},${learningCaseId},${dayClosed.id},'day_close_observation','Day Close에서 실제 실행 결과를 관찰함',
            ${tx.json(input.dayCloseResult as JSONValue)},${input.observedAt})
        `;
        created += 1;
      }
      return created;

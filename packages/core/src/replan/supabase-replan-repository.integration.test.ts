...
const rejectedUser = randomUUID() as UserId;
const expiredUser = randomUUID() as UserId;
...
describe("Supabase dynamic replanning", () => {
  ...
  it("creates an important proposed revision and approves it through the durable approval", async () => {
    ...
    const after = await sql<{ revision_no: number; status: string; approved_count: number; approval_count: number }[]>`
      select
  ...
  it("keeps the approved plan when an important replacement is rejected", async () => {
    ...
    const plans = await sql<{ revision_no: number; status: string }[]>`

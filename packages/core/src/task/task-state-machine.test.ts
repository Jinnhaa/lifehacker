import type { DomainError } from "@amber/shared";
    ...
    for (const next of taskStatuses) {
      ...
      it(`${previous} -> ${next} is ${expected ? "allowed" : "rejected"}`, () => {
        ...
        if (expected) {
          ...
          expect(() => assertTaskTransition(previous, next)).toThrowError(
            expect.objectContaining<Partial<DomainError>>({ code: "INVALID_TASK_TRANSITION" })
          );
        }
      });
    }
...
describe("Task transition event mapping", () => {
  ...
  ] as const)("maps %s -> %s to %s", (previous, next, eventType) => {

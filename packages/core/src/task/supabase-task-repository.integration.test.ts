...
import { FixedClock, type CorrelationId, type IdGenerator, type TaskId, type UserId } from "@amber/shared";
...
const eventFor = (taskId: TaskId, userId: UserId, idempotencyKey?: string) => ({
  userId,
  aggregateId: taskId,
  ...
  payload: {
    previous_status: "INBOX" as const,
    next_status: "IN_PROGRESS" as const,
    source: "test",
    changed_at: "2026-09-03T01:00:00.000Z"
  }
});

const createTask = (userId: UserId, title: string) =>
  repository.createTask(
    { userId, title, executionMode: "standard", importance: 3 },
    {
      userId,
...
describe("SupabaseTaskRepository", () => {
  ...
  it("rolls back the state update when event insertion fails", async () => {
    ...
    await expect(
      repository.transitionTask(userA, task.id, "INBOX", "IN_PROGRESS", null, eventFor(task.id, userA, duplicateKey))
...
describe("TaskService", () => {
  it("validates, transitions, and emits exactly one event per successful state change", async () => {
    ...
    const events = await admin<{ event_type: string; payload: { changed_at: string } }[]>`

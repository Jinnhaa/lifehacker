...
import type { WakeRepository, WakeTargetContext, WakeWorkflowRun } from "./wake.js";

const userId = "10000000-0000-4000-8000-000000000001" as UserId;
const now = new Date("2026-09-04T00:00:00.000Z"); // 09:00 Asia/Seoul
const emptyContext: WakeTargetContext = { preferredWakeTime: null, preferenceSource: null, firstConstraint: null };

const run = (targetDate: string, timeZone = "Asia/Seoul"): WakeWorkflowRun => ({
...
const repository = (): WakeRepository => ({
...
const message = (text: string, receivedAt = now) => ({
...
describe("WakeWorkflowService", () => {
  ...
  it.each([["10분만", 10], ["20분 뒤에", 20], ["30분 뒤에", 30], ["조금 있다가 깨워줘", 10]])(
    "schedules snooze for %s", async (text, minutes) => {
      const repo = repository();
      const result = await new WakeWorkflowService({ repository: repo, clock: new FixedClock(now) })
        .handleWakeMessage(message(text));

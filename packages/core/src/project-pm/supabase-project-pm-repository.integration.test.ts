...
describe("SupabaseProjectPmRepository", () => {
  it("bootstraps built-in instances idempotently by user, type, and scope", async () => {
    ...
    const counts = await sql<{ user_id: string; template_key: string; count: number }[]>`
  ...
  it("loads only project WorkContexts and keeps project facts isolated by user and project", async () => {
    ...
    const context = await repository.loadProjectContext(
      userId, selected, "2026-09-06", "Asia/Seoul", new Date("2026-09-06T03:00:00Z")
    );
  ...
  it("records one scoped agent execution trace idempotently", async () => {
    ...
    const rows = await sql<{
      run_count: number; package_count: number; artifact_count: number; scope_matches: boolean; correlated_runs: number;
    }[]>`
      select
    ...
    const bootstrapped = await sql<{ template_key: string; home_scope_id: string; count: number }[]>`
  ...
  it("persists the project observation loop without mutating canonical work", async () => {
    const before = await sql<{ tasks: number; steps: number; objectives: number }[]>`
      select
    ...
    const analysisProvider = {
      reviewProjectState: vi.fn(async () => ({
        gaps: [{
          ...
          evidenceRefs: [`task:${taskId}`], priorityHint: "high" as const, confidence: 0.9, blocking: true,
    ...
    const persisted = await sql<{ artifact_type: string; work_context_id: string; content_hash: string }[]>`
    ...
    const traces = await sql<{ status: string; current_step: string; checkpoint_version: number; work_context_id: string; package_count: number }[]>`
    ...
    const after = await sql<{ tasks: number; steps: number; objectives: number }[]>`
      select
        (select count(*)::int from public.tasks where user_id=${userId}) tasks,
        (select count(*)::int from public.task_steps where user_id=${userId}) steps,
        (select count(*)::int from public.objectives where user_id=${userId}) objectives
    ...
    const otherArtifacts = await sql<{ count: number }[]>`

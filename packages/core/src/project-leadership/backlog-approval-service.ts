...
import {
  ...
  type BacklogApprovalDecision,
  type BacklogApprovalRepository,
  type BacklogApprovalRequestInput,
...
const validateDecision = (approval: Awaited<ReturnType<BacklogApprovalRepository["getApproval"]>>, decision: BacklogApprovalDecision): void => {
...
export class BacklogApprovalService {
  ...
  async requestApproval(input: BacklogApprovalRequestInput): Promise<Awaited<ReturnType<BacklogApprovalRepository["requestApproval"]>>> {
    return this.dependencies.repository.requestApproval({ ...input, now: this.dependencies.clock.now() });
  }

  async decide(input: unknown, timeZone: string): Promise<BacklogApprovalResult> {
    ...
    if (!project) throw new DomainError("CONFLICT", "Project state is no longer available");
    const now = this.dependencies.clock.now();
    const context = await this.dependencies.projectRepository.loadProjectContext(
      decision.userId, project, localDate(now, timeZone), timeZone, now
    );

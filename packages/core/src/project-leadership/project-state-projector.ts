...
import type { ProjectPmContext, ProjectSourceReference } from "../project-pm/project-pm.js";
import { projectStateSnapshotSchema, type ProjectStateSnapshot } from "./project-leadership.js";

const byId = <T extends { readonly id: string }>(left: T, right: T): number => left.id.localeCompare(right.id);
...
export const projectProjectState = (context: ProjectPmContext): ProjectStateSnapshot => {
  const objectives = [...context.objectives].sort((left, right) =>
  ...
  const internalSource = (kind: ProjectStateSnapshot["sourceRefs"][number]["kind"], id: string, hash: string | null = null) => ({
    ref: `${kind}:${id}`, kind, freshness: "current" as const, observedAt: generatedAt, contentHash: hash
  });
  const sourceRefs = [

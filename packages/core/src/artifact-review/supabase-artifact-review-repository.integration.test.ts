...
import type { ArtifactReviewCommand } from "./artifact-review.js";
...
describe("SupabaseArtifactReviewRepository", () => {
  it("atomically accepts the Artifact, completes linked steps and Task, records Why, and deduplicates replay", async () => {
    ...
    const [artifacts, tasks, steps, feedback] = await Promise.all([
      sql<{ review_status: string }[]>`select review_status from public.artifacts where id=${artifactId}`,
      sql<{ status: string }[]>`select status from public.tasks where id=${taskId}`,
      sql<{ status: string }[]>`select status from public.task_steps where task_id=${taskId} order by position`,
      sql<{ user_reason: string | null }[]>`select f.user_reason from public.decision_feedback f join public.decisions d on d.id=f.decision_id where d.id=${first.decisionId}`
    ]);

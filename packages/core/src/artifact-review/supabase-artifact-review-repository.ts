...
interface TargetRow {
  artifact_id: string; content_hash: string; content_text: string; verification_status: string | null; review_status: string | null;
  work_context_id: string; task_id: string; task_status: string; execution_mode: string; task_step_id: string;
  step_status: string; position: number; completion_criteria: string | null; source_agent_run_id: string;
}

const mapTarget = (userId: UserId, row: TargetRow): ArtifactReviewTarget => ({
...
export class SupabaseArtifactReviewRepository implements ArtifactReviewRepository {
  constructor(private readonly sql: Sql) {}

  async loadTarget(userId: UserId, artifactId: string): Promise<ArtifactReviewTarget | null> {
  ...
  async recordReview(input: { readonly command: ArtifactReviewCommand; readonly target: ArtifactReviewTarget; readonly now: Date }): Promise<ArtifactReviewRecord> {
    ...
    return this.sql.begin(async (tx) => {
      ...
      const duplicate = await tx<{ id: string; correlation_id: string; payload: Record<string, unknown> }[]>`
      ...
      if (!current || current.work_context_id !== command.workContextId) throw new DomainError("CROSS_USER_ACCESS", "Artifact is outside the requested Project scope");
      if (current.content_hash !== command.artifactContentHash) throw new DomainError("CONFLICT", "Artifact content changed before review");
      if (current.review_status !== "pending_review") throw new DomainError("CONFLICT", "Artifact has already been reviewed");
      const runRows = await tx<{ workflow_run_id: string; correlation_id: string }[]>`
      ...
      `;
      const run = runRows[0];
      if (!run) throw new DomainError("CONFLICT", "Artifact execution trace is incomplete");
      ...
      if (command.decision === "accept") {
        ...
        if (current.execution_mode === "mixed") {
          const reviewSteps = await tx<{ id: string }[]>`
      ...
      const previousStatus = current.task_status as TaskStatus;
      const nextStatus: TaskStatus = taskCompleted ? "DONE" : command.decision === "reject" ? "BLOCKED" : "IN_PROGRESS";
      ...
      const causation = await tx<{ id: string }[]>`
      ...
      `;
      const reviewEvents = await tx<{ id: string }[]>`

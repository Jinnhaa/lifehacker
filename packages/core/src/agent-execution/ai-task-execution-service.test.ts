...
import type { ProjectPmContext, ProjectPmRepository } from "../project-pm/project-pm.js";
import type { AiExecutionAttempt, AiTaskExecutionRepository, AiTaskExecutionTarget, AiTaskExecutor, DocumentDraftResult } from "./ai-task-execution.js";
...
const taskId = "10000000-0000-4000-8000-000000000002" as TaskId;
...
const target = (overrides: Partial<AiTaskExecutionTarget> = {}): AiTaskExecutionTarget => ({
...
const validOutput = (): DocumentDraftResult => ({
  title: "Draft", summary: "Summary", body: "Body", addressedCriteria: ["criterion-a"],
  sourceRefs: [`task_step:${stepId}`], uncertainties: []
});

class MemoryRepository implements AiTaskExecutionRepository {
  attempts: AiExecutionAttempt[] = [];
  completed: { agentRunId: string; artifactId: string } | null = null;
  failures: { terminal: boolean }[] = [];
  blocked = 0;
  constructor(public targetValue: AiTaskExecutionTarget | null = target()) {}
  async loadTarget() { return this.targetValue; }
  async findSuccessful() { return this.completed; }
  async startAttempt(): Promise<AiExecutionAttempt> {
    const number = this.attempts.length + 1;
    const attempt = { kind: "started" as const, workflowRunId: `20000000-0000-4000-8000-00000000000${number}`, agentRunId: `30000000-0000-4000-8000-00000000000${number}`, contextPackageId: `40000000-0000-4000-8000-00000000000${number}`, attemptNumber: number };
    this.attempts.push(attempt); return attempt;
  }
  async completeAttempt(input: { attempt: AiExecutionAttempt }) { this.completed = { agentRunId: input.attempt.agentRunId, artifactId: "50000000-0000-4000-8000-000000000001" }; return this.completed.artifactId; }
  async failAttempt(input: { terminal: boolean }) { this.failures.push({ terminal: input.terminal }); }
  async blockWithoutAttempt() { this.blocked += 1; }
}

const setup = (options: { target?: AiTaskExecutionTarget; projectContext?: ProjectPmContext; outputs?: readonly (unknown | Error)[] } = {}) => {
  ...
  const executor: AiTaskExecutor = { executeDocumentDraft: vi.fn(async () => {

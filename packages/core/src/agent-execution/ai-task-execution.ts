...
export interface AiTaskExecutionRepository {
  ...
  completeAttempt(input: { readonly target: AiTaskExecutionTarget; readonly attempt: AiExecutionAttempt; readonly executionKey: string; readonly result: DocumentDraftResult; readonly sourceRefs: readonly string[]; readonly now: Date }): Promise<string>;
...
export const createExecutionContext = (context: ProjectPmContext, taskStepId: string): Omit<DocumentDraftInput, "workflowRunId" | "agentRunId" | "contextPackageId"> => {

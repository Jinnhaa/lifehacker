...
import {
  ...
  type AiTaskExecutionRepository,
...
export class AiTaskExecutionService {
  ...
  async dispatch(input: {
    ...
    if (target.status === "dependency_waiting") throw new DomainError("CONFLICT", "TaskStep dependencies are unresolved");
    ...
    if (successful) return { status: "reused", ...successful };
    ...
    for (let index = 0; index < MAX_ATTEMPTS; index += 1) {
      ...
      if (started.kind === "reused") return { status: "reused", agentRunId: started.agentRunId!, artifactId: started.artifactId! };
      if (started.kind === "busy") return { status: "busy" };
      if (started.kind === "exhausted") return { status: "blocked" };
      if (started.kind !== "started") return { status: "blocked" };
      const attempt: AiExecutionAttempt = started;
      try {
        ...
        if (terminal) return { status: "blocked" };
      }
    }
    return { status: "blocked" };
  }

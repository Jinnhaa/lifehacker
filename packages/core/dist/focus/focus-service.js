...
export const classifyBlockReason = (text) => {
    const normalized = text.trim().toLowerCase();
    if (/^(unclear|hard|avoidance|perfectionism|missing_material|other)$/.test(normalized))
        return normalized;
    if (/(자료|파일|권한|답변|사람|연락|준비물).*(없|필요|못)|(없|필요).*(자료|파일|권한|답변|사람|연락|준비물)/.test(normalized))
        return "missing_material";
    if (/(완벽|퀄리티|품질|잘해야)/.test(normalized))
        return "perfectionism";
    if (/(하기 싫|미루|손이 안|귀찮|회피)/.test(normalized))
        return "avoidance";
    if (/(어려|이해 안|모르겠는데.*어렵|난이도)/.test(normalized))
        return "hard";
    if (/(뭘 해야|무엇을 해야|불명확|애매|다음 행동|모르겠)/.test(normalized))
        return "unclear";
    if (/(기타|다른 이유)/.test(normalized))
        return "other";
    return null;
};
...
const duration = (minutes) => minutes < 60
    ? `${minutes}분`
    : minutes % 60 === 0 ? `${minutes / 60}시간` : `${Math.floor(minutes / 60)}시간 ${minutes % 60}분`;
const formatFocus = (context, includeFlow) => {
    const details = [
    ...
    else if (context.nextAction)
        details.push(`지금 행동: ${context.nextAction}`);
    return details.join("\n\n");
};
const nextActionText = (action) => action
    ? `다음 할 일은 ${action.title}이야. 시작하려면 “시작”이라고 보내줘.`
    : "다음 Current Action은 없어.";
const formatCompletion = (result) => result.kind === "next_step"
    ? `Step을 완료했어.\n\n${formatFocus(result.context, false)}`
    : `${result.taskTitle}을 완료했어.\n\n${nextActionText(result.nextAction)}`;
export const formatRecovery = (result) => {
...
export class FocusWorkflowService {
    ...
    async handleFocusMessage(message) {
        ...
        if (workflow?.currentStep === "awaiting_block_reason")
            return this.handleBlockReason(message);
        if (workflow?.currentStep === "awaiting_missing_detail")
            return this.finishDetailedBlock(message, planDate, "missing_material");
        if (workflow?.currentStep === "awaiting_other_detail")
            return this.finishDetailedBlock(message, planDate, "other");
        ...
        if (text === "시작" || text === "시작할게" || timedStart) {
            ...
            if (durationMinutes < 1 || durationMinutes > 240)
                return { handled: true, reply: "집중 시간은 1–240분으로 설정해 줘." };
            ...
            const target = timedStart?.[2] && timedStart[3]
                ? { kind: timedStart[2], id: timedStart[3] }
                : undefined;
            ...
            if (!result.action)
                return { handled: true, reply: "지금 시작할 Current Action이 없어." };
            if (result.action.kind === "rest")
                return { handled: true, reply: `현재 행동은 ${result.action.title}이야. 집중 세션은 만들지 않았어.` };
            if (!result.context)
                return { handled: true, reply: "지금은 Focus를 시작하지 못했어." };
    ...
    async handleBlockReason(message) {
        const category = classifyBlockReason(message.text);
        if (!category)
            return { handled: true, reply: BLOCK_QUESTION };
        ...
        const result = await this.repository.recordBlock(message.userId, localDate(message.receivedAt, message.timeZone), category, message.text, this.clock.now(), message.messageId);
        return { handled: true, reply: result ? formatRecovery(result) : "막힘을 기록할 active Focus가 없어." };
    }
    async finishDetailedBlock(message, planDate, category) {

...
const strings = (value) => Array.isArray(value)
    ...
    : [];
const situationType = (item) => {
    const tags = [...strings(item.situation.impactReasons), ...strings(item.situation.reasons)];
    if (tags.length > 0)
        return [...new Set(tags)].sort().join("+");
    if (item.decisionType === "focus_task_switch")
        return "focus_guardrail";
    if (item.decisionType === "important_replan")
        return "important_change";
    if (item.decisionType === "morning_override")
        return "morning_priority_override";
    return "unspecified";
};
const reasonType = (reason) => {
    if (!reason)
        return null;
    if (/(마감|기한|급한|deadline)/i.test(reason))
        return "deadline_priority";
    if (/(루틴|습관|운동|학습)/.test(reason))
        return "routine_priority";
    if (/(피곤|수면|잠|체력)/.test(reason))
        return "fatigue";
    if (/(집중|몰입|산만)/.test(reason))
        return "focus_difficulty";
    if (/(일정|약속|회의|수업)/.test(reason))
...
const outcomeType = (outcome) => {
    ...
    if (Array.isArray(outcome.blockedTaskIds) && outcome.blockedTaskIds.length > 0)
        return "task_blocked";
    if (Array.isArray(outcome.completedTaskIds) && outcome.completedTaskIds.length > 0)
        return "task_completed";
    if (typeof outcome.varianceMinutes === "number") {
        if (outcome.varianceMinutes < 0)
            return "below_planned_time";
        if (outcome.varianceMinutes > 0)
            return "above_planned_time";
        return "matched_planned_time";
    }
    return null;
};
const contextText = (decisionType, situation) => {
    if (situation.includes("deadline"))
        return "마감 위험이 있는 중요한 일정 변경에서";
    if (situation.includes("protected_routine"))
        return "보호된 루틴이 영향을 받는 일정 변경에서";
    if (decisionType === "focus_task_switch")
        return "Focus 전환 확인 이후";
    if (decisionType === "morning_override")
        return "Morning 계획의 중요한 우선순위를 바꿀 때";
    return "중요한 일정 변경에서";
};
const choiceText = (action) => ({
    approve: "Amber 제안을 승인하는 선택",
    reject: "Amber 제안을 거절하는 선택",
    exclude_tasks: "제안된 Task를 제외하는 선택",
    switch_task: "현재 Task에서 전환하는 선택"
}[action] ?? `“${action}” 선택`);
const signalText = (kind, value) => {
    const labels = {
...
const confidence = (evidence) => {
    const outcomes = evidence.map((item) => outcomeType(item.observedOutcome)).filter((value) => value !== null);
...
export const derivePatternCandidates = (cases) => {
    ...
    for (const [base, items] of bases) {
        const [decisionType, situation, action] = base.split("|");
        ...
        const reasonCaseIds = new Set();
        const qualifyingReasons = [...reasonGroups.entries()].filter(([, evidence]) => evidence.length >= MIN_PATTERN_EVIDENCE);
        for (const [, evidence] of qualifyingReasons)
            evidence.forEach((item) => reasonCaseIds.add(item.id));
        const groups = qualifyingReasons
            .map(([value, evidence]) => ["reason", value, evidence]);
        ...
        for (const item of items.filter((entry) => !reasonCaseIds.has(entry.id))) {
            ...
            if (outcome)
                outcomeGroups.set(outcome, [...(outcomeGroups.get(outcome) ?? []), item]);
        }
        groups.push(...[...outcomeGroups.entries()]
            .filter(([, evidence]) => evidence.length >= MIN_PATTERN_EVIDENCE)
            .map(([value, evidence]) => ["outcome", value, evidence]));
        for (const [kind, value, evidence] of groups) {
            if (decisionType.startsWith("execution_") && new Set(evidence.map(e => e.observedOutcome.date)).size < MIN_PATTERN_EVIDENCE)
                continue;
            if (decisionType === "execution_estimate" && new Set(evidence.map(e => e.observedOutcome.taskId)).size < MIN_PATTERN_EVIDENCE)
                continue;
            candidates.push({

import { derivePatternCandidates, PATTERN_EVALUATOR_VERSION } from "./pattern-learning.js";
const object = (value) => value !== null && typeof value === "object" && !Array.isArray(value)
    ? value
    : {};
const mapCase = (row) => {
    const snapshot = object(row.context_snapshot);
    if (typeof snapshot.observationKey === "string")
        return {
...
export class SupabasePatternLearningRepository {
    sql;
    constructor(sql) {
        this.sql = sql;
    }
    async evaluatePatterns(userId) {
      ...
      `;
        ...
        `;
                if (existing[0] && count[0].count > existing[0].evidence_count)
                    strengthened += 1;
            }
...
//# sourceMappingURL=supabase-pattern-learning-repository.js.map
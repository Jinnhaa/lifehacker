...
const mapWorkflow = (row) => {
    ...
    return {
        ...
        impact: String(checkpoint.impact),
        ...(Array.isArray(checkpoint.impactReasons)
            ? { impactReasons: checkpoint.impactReasons.filter((value) => typeof value === "string") }
            : {}),
        ...(typeof checkpoint.lastMessageId === "string" ? { lastMessageId: checkpoint.lastMessageId } : {})
    };
};
const parseIntervals = (value) => Array.isArray(value) ? value.flatMap((entry) => {
...
const actionHash = (draft) => createHash("sha256").update(JSON.stringify(draft.items.map((item) => ({
...
const periodKey = (date, weekStartsOn) => addDays(date, -((weekday(date) - weekStartsOn + 7) % 7));
export class SupabaseReplanRepository {
    sql;
    constructor(sql) {
        this.sql = sql;
    }
    async findLatestPendingTrigger(userId) {
    ...
    `;
        if (!existing[0])
            throw new Error("Manual replan trigger disappeared");
        return mapTrigger(existing[0]);
    }
    async findPendingApproval(userId, planDate) {
        const rows = await this.sql `
    ...
    async findCompletedApprovalByMessage(userId, planDate, messageId) {
        const rows = await this.sql `
      select * from public.workflow_runs where user_id=${userId} and workflow_type='dynamic_replanning'
    ...
    async findByTrigger(userId, triggerId) {
        const rows = await this.sql `
      select * from public.workflow_runs where user_id=${userId} and workflow_type='dynamic_replanning'
    ...
    async loadPlanState(userId, planDate) {
        const plans = await this.sql `
    ...
    `;
        ...
        if (Number.isNaN(workUntil.getTime()))
            throw new Error("Approved plan is missing workUntil");
        return {
            planId: plan.id,
    ...
    async createRevision(trigger, stateHash, previous, draft, decision, now) {
      ...
      `;
            if (existing[0]) {
                ...
                if (!plan)
                    throw new Error("Replan workflow plan missing");
      ...
      `;
        ...
        `;
                if (concurrent[0]) {
                    ...
                    const plan = await this.getPlan(tx, trigger.userId, workflow.planId);
                    if (!plan)
                        throw new Error("Concurrent replan plan missing");
                    await this.recordTriggerExecution(tx, trigger, workflow.id, plan.id, now);
                    return { workflow, plan, duplicate: true };
                }
      ...
      `;
        ...
        values(${trigger.userId},${previous.planDate},${previous.timeZone},${revisionNo},'pending_approval',${previous.planId},
          ${tx.json(snapshot)},'dynamic_replanning') returning id,plan_date::text,timezone,revision_no,status,input_snapshot
      `;
    ...
    async loadItems(sql, userId, planId) {
    ...
    async recordTriggerExecution(sql, trigger, workflowId, planId, now) {
        await sql `
    ...
    async insertItems(sql, userId, planDate, planId, draft) {
        const settings = await sql `select week_starts_on from public.user_settings where user_id=${userId}`;
    ...
    `;
        ...
        return {
            id: plan.id, revisionNo: plan.revision_no, status: plan.status, items: planItems, fixedEvents,
            highlights: Array.isArray(snapshot.highlights) ? snapshot.highlights.filter((value) => typeof value === "string") : []
        };
    }
}
//# sourceMappingURL=supabase-replan-repository.js.map
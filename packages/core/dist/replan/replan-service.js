...
const formatPlan = (plan, timeZone) => plan.items.length === 0
    ? "배치할 수 있는 남은 일이 없어."
...
const stateHash = (trigger, state, observation) => createHash("sha256")
...
export class DynamicReplanningService {
    ...
    async handleReplanMessage(message) {
    ...
    async recordImportantDecision(message, workflow, choice) {
    ...
    async processTrigger(trigger, timeZone, receivedAt) {
        const prior = await this.repository.findByTrigger(trigger.userId, trigger.id);
        if (prior)
            return prior.workflow.impact === "SMALL_CHANGE"
                ? `이미 일정을 조정했어.\n\n${formatPlan(prior.plan, prior.workflow.timeZone)}`
                : `새 계획을 이미 제안했어.\n\n${formatPlan(prior.plan, prior.workflow.timeZone)}\n\n“승인”이라고 보내면 반영할게.`;
        if (!shouldReplanForTrigger(trigger.reason, trigger.deltaMinutes))
            return null;
...
//# sourceMappingURL=replan-service.js.map
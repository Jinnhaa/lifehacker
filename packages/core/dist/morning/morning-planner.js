...
const mergeIntervals = (intervals, horizon) => {
    ...
    for (const value of clipped) {
        ...
        if (!previous || value.start > previous.end)
            merged.push(value);
        else if (value.end > previous.end)
            merged[merged.length - 1] = { start: previous.start, end: value.end };
    }
    return merged;
};
const freeIntervals = (horizon, occupied) => {
    const free = [];
    ...
    if (cursor < horizon.end)
        free.push({ start: cursor, end: horizon.end });
    return free;
};
const containsId = (value, id) => {
    ...
    if (Array.isArray(value))
        return value.some((entry) => containsId(entry, id));
    if (value && typeof value === "object")
        return Object.values(value).some((entry) => containsId(entry, id));
    return false;
};
const remainingSuitableDays = (activity, localWeekday) => {
    const days = activity.preferredDays;
    if (!days || days.length === 0)
        return 8 - localWeekday;
...
const allocate = (intervals, candidate, maximumMinutes) => {
    ...
    if (target < candidate.minimumMinutes)
        return { items: [], used: 0 };
    const items = [];
    ...
    while (remaining > 0 && intervals.length > 0) {
        ...
        if (end >= interval.end)
            ...
            intervals[0] = { start: end, end: interval.end };
    }
    return { items, used: target - remaining };
};
export const createMorningPlan = (input) => {
    ...
    if (bufferMinutes > 0) {
        const buffer = { type: "task", id: "", title: "버퍼", minutes: bufferMinutes, minimumMinutes: 1, rank: 0, importance: 1, deadline: null };
...
//# sourceMappingURL=morning-planner.js.map
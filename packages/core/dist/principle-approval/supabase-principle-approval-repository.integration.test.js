...
const addPattern = async (owner, action, evidenceCount) => {
...
const message = (text, messageId) => ({ userId, text, messageId, receivedAt: now });
describe("Supabase Principle Approval", () => {
    ...
    it("preserves an edit as a new pending revision, then approves it idempotently", async () => {
        const revised = await service.handlePrincipleApprovalMessage(message("수정: 시험이나 발표가 24시간 이내일 때만", "discord:revise"));
...
//# sourceMappingURL=supabase-principle-approval-repository.integration.test.js.map
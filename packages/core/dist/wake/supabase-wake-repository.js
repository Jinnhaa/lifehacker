...
const checkpointJson = (value) => value;
export class SupabaseWakeRepository {
    sql;
    constructor(sql) {
        this.sql = sql;
    }
    async findWorkflow(userId, targetDate) {
    ...
    async getOrCreateAwaitingWorkflow(userId, targetDate, timeZone, context, now) {
        const checkpoint = {
            targetDate,
            timeZone,
    ...
    async loadTargetContext(userId, targetDate, timeZone) {
        ...
        const [settings, preferences, personalConstraints, fixedConstraints] = await Promise.all([
            this.sql `select wake_policy from public.user_settings where user_id=${userId}`,
            this.sql `
    ...
    async schedule(run, wakeAt, source, messageId, now) {
        return this.sql.begin(async (tx) => {
            ...
            if (current.checkpoint.lastMessageId === messageId && current.currentStep === "scheduled")
                return current;
            await tx `
    ...
    async acknowledgeForDate(userId, targetDate, messageId, now) {
      ...
      `;
            ...
            if (run.currentStep === "acknowledged")
                return;
            await tx `
    ...
    async snooze(userId, targetDate, wakeAt, messageId, now) {
        return this.sql.begin(async (tx) => {
            const rows = await tx `
        select * from public.workflow_runs where user_id=${userId} and workflow_type='wake'
          and idempotency_key=${`wake:${targetDate}`} for update
      `;
            if (!rows[0])
                return false;
            const run = mapWorkflow(rows[0]);
            if (run.checkpoint.lastMessageId === messageId)
                return true;
            const sent = await tx `
    ...
    async completeDelivery(claim, now) {
        await this.sql.begin(async (tx) => {
            const jobs = await tx `
    ...
    async failDelivery(claim, error, now) {
        await this.sql `
...
//# sourceMappingURL=supabase-wake-repository.js.map
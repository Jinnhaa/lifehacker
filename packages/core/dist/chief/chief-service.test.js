...
const serviceWith = (loaded, recorder, projectPm, workstyleResolver) => {
...
describe("ChiefAgentService", () => {
    ...
    it.each(["LogFolio 현황 봐줘", "NEXTiME 뭐 남았어?", "LogFolio에서 지금 뭐 해야 돼?", "LogFolio 프로젝트 상태 알려줘"])("delegates the project request %s to Project PM", async (text) => {
        const projectPm = { getProjectReport: vi.fn(async () => ({ handled: true, reply: "pm", report: projectReport })) };
    ...
    it("reports only structured Project PM facts", async () => {
        const projectPm = { getProjectReport: vi.fn(async () => ({ handled: true, reply: "raw", report: projectReport })) };
    ...
    it("contains Project PM failures without crashing Chief", async () => {
        const projectPm = { getProjectReport: vi.fn(async () => { throw new Error("internal"); }) };
    ...
    it("lets the current explicit instruction override stored response style", async () => {
        const resolver = { resolve: vi.fn(async (input) => ({
    ...
    it("keeps general Chief requests on the direct path", async () => {
        const projectPm = { getProjectReport: vi.fn(async () => ({ handled: false })) };
        const { service, reader } = serviceWith(context(), undefined, projectPm);

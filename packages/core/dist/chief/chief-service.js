...
const requestKind = (text) => {
    ...
    if (STATUS_REQUESTS.has(normalized))
        return "status";
    if (NEXT_ACTION_REQUESTS.has(normalized))
        return "next_action";
    return null;
};
...
const taskDeadline = (task) => task.internalDeadline ?? task.officialDeadline;
const remainingSuitableDays = (preferredDays, now, timeZone, weekStartsOn) => {
...
const routineRisks = (context) => context.observation.recurringActivities.flatMap((activity) => {
...
const taskRank = (task, context) => {
    ...
    if (isOverdue(deadline, context.observedAt))
        return 1;
...
const actionLine = (title, minutes) => minutes && minutes > 0 ? `${title} · 예상 ${minutes}분` : title;
const projectDateLabel = (value, timeZone) => {
...
const formatDelegatedProject = (report, timeZone, workstyle) => {
    const lines = [
    ...
    if (report.blockers.length > 0)
        lines.push(`${report.blockers.slice(0, 3).join(", ")}은 아직 막혀 있어.`);
    if (includesReasoning(workstyle) && report.nextAction) {
        lines.push("", "근거", "현재 진행 상태와 가장 가까운 마감을 기준으로 바로 실행할 일을 골랐어.");
    }
    if (report.warnings.length > 0)
        lines.push("", "주의", ...report.warnings);
    return lines.join("\n");
};
const currentActionSelection = (context) => {
...
const buildReply = (context, kind, workstyle) => {
    ...
    if (kind === "next_action") {
        ...
        if (warning)
            lines.push("", "주의", warning);
        if (principleExplanation)
            lines.push("", principleExplanation);
        if (includesReasoning(workstyle) && !principleExplanation && selected)
            lines.push("", "근거", "현재 실행 상태와 마감·중요도를 기준으로 골랐어.");
    ...
    if (context.replannedToday)
        lines.push("- 오늘 일정 조정됨");
    if (risks.some((risk) => risk.risk === "HIGH"))
        lines.push(`- 반복 활동 주의 ${risks.filter((risk) => risk.risk === "HIGH").length}개`);
...
export class ChiefAgentService {
    ...
    async handleChiefMessage(message) {
        const projectRequest = parseProjectPmRequest(message.text);
        if (projectRequest && this.dependencies.projectPm)
            return this.delegateProject(message, projectRequest);
        const kind = requestKind(message.text);
        if (!kind)
            return { handled: false };
        const previous = await this.findPrevious(message);
        if (previous)
            return { handled: true, reply: previous.reply };
        ...
        const context = await this.dependencies.contextReader.loadChiefContext(message.userId, localDate(message.receivedAt, message.timeZone), message.timeZone, startedAt);
        if (context.userId !== message.userId)
            throw new Error("Chief context owner mismatch");
    ...
    async delegateProject(message, request) {
        const previous = await this.findPrevious(message);
        if (previous)
            return { handled: true, reply: previous.reply };
        const startedAt = this.dependencies.clock.now();
        ...
        try {
            ...
            if (!result.report)
                return { handled: true, reply: result.reply ?? "어느 프로젝트를 확인할지 정확한 이름을 알려줘." };
...
export const isChiefRequest = (text) => requestKind(text) !== null || parseProjectPmRequest(text) !== null;
//# sourceMappingURL=chief-service.js.map
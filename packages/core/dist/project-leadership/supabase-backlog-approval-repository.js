...
const priorityImportance = { low: 2, medium: 3, high: 4, critical: 5 };
const roughMinutes = { xs: 15, s: 30, m: 60, l: 120, xl: 240 };
export class SupabaseBacklogApprovalRepository {
    sql;
    constructor(sql) {
        this.sql = sql;
    }
    async requestApproval(input) {
        const workflowRunId = await this.sql.begin(async (tx) => {
            ...
            const proposalApproval = await tx `
      ...
      `;
            if (proposalApproval[0])
                return proposalApproval[0].workflow_run_id;
      ...
      `;
            ...
            if (!proposalRow || !proposalRow.work_context_id)
                throw new DomainError("INVALID_INPUT", "Backlog proposal artifact not found");
            if (proposalRow.content_hash !== input.proposalHash)
                throw new DomainError("CONFLICT", "Backlog proposal hash does not match");
            const proposal = backlogProposalArtifactSchema.parse(JSON.parse(proposalRow.content_text));
            if (proposal.workContextId !== proposalRow.work_context_id)
                throw new DomainError("CONFLICT", "Backlog proposal scope does not match its artifact");
      ...
      `;
            ...
            if (!snapshotRow)
                throw new DomainError("CONFLICT", "Backlog proposal source snapshot is missing");
      ...
      `;
            if (!sourceRuns[0])
                throw new DomainError("CONFLICT", "Backlog proposal source workflow is missing");
            const checkpoint = {
      ...
      `;
        ...
        if (!approval)
            throw new Error("Created backlog approval disappeared");
    ...
    async getApproval(userId, approvalRequestId) {
    ...
    async expireStale(userId, approvalRequestId, now) {
      ...
      `;
        ...
        `;
                const steps = owner === "hybrid"
                    ? [{ title: `초안 작성: ${item.title}`, owner: "ai" }, { title: `검토 및 확정: ${item.title}`, owner: "user" }]
                    : [{ title: item.title, owner: owner === "human" ? "user" : "ai" }];
                for (const [index, step] of steps.entries()) {
                    const stepId = stableUuid(`backlog-step:${decision.proposalArtifactId}:${item.key}:${index + 1}`);
                    await tx `
            insert into public.task_steps(id,user_id,task_id,position,title,owner,estimated_minutes,completion_criteria,status,skill_key,review_of_step_id)
            values(${stepId},${decision.userId},${taskId},${index + 1},${step.title},${step.owner},
              ...
              ${owner === "hybrid" && index === 1 ? stableUuid(`backlog-step:${decision.proposalArtifactId}:${item.key}:1`) : null})
            on conflict(id) do nothing
          `;
                    await tx `
            insert into public.domain_events(user_id,event_type,aggregate_type,aggregate_id,actor_type,occurred_at,
    ...
    async getApprovalByWorkflow(userId, workflowRunId) {
        const rows = await this.sql `
      select a.id,a.user_id,a.workflow_run_id,a.action_ref,a.action_hash,a.checkpoint_version,a.status,w.checkpoint_state
      from public.approval_requests a join public.workflow_runs w on w.id=a.workflow_run_id and w.user_id=a.user_id
    ...
    `;
        if (!proposalRows[0] || !snapshotRows[0])
            throw new Error("Backlog approval source artifact is missing");
        return {
            id: row.id,
            userId: row.user_id,
    ...
    async loadMaterializedResult(userId, workflowRunId) {
        const rows = await this.sql `
    ...
    `;
        ...
        if (!workflow || typeof workflow.checkpoint_state.decisionId !== "string")
            throw new Error("Materialized backlog result is missing");
        const taskIds = Array.isArray(workflow.checkpoint_state.taskIds)
            ? workflow.checkpoint_state.taskIds.filter((value) => typeof value === "string") : [];
        const tasks = taskIds.length === 0 ? [] : await this.sql `
      select id,title,importance from public.tasks where user_id=${userId} and id in ${this.sql(taskIds)} order by created_at,id
    `;
        const stepRows = taskIds.length === 0 ? [] : await this.sql `
    ...
    `;
        ...
        const acceptedItemKeys = Array.isArray(workflow.checkpoint_state.acceptedItemKeys)
            ? workflow.checkpoint_state.acceptedItemKeys.filter((value) => typeof value === "string") : [];
        const dependenciesByItem = workflow.checkpoint_state.dependenciesByItem && typeof workflow.checkpoint_state.dependenciesByItem === "object"
            ? workflow.checkpoint_state.dependenciesByItem : {};
        ...
        const excludedItemKeys = Array.isArray(workflow.checkpoint_state.excludedItemKeys)
            ? workflow.checkpoint_state.excludedItemKeys.filter((value) => typeof value === "string") : [];
        return {
...
//# sourceMappingURL=supabase-backlog-approval-repository.js.map
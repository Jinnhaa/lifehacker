...
const backlogRefinementInputSchema = z.object({
    userId: userIdSchema, workflowRunId: z.string().min(1), snapshot: projectStateSnapshotSchema,
    gapAnalysis: gapAnalysisArtifactSchema, constraints: z.array(z.string())
});
export const projectStateReviewSkill = {
...
export const backlogRefinementSkill = {
    ...
    verify(input, output) {
        ...
        for (const gap of input.gapAnalysis.gaps)
            evidence.add(`gap:${gap.key}`);

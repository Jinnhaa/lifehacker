...
const task = (id, status) => ({
    id: id, userId, workContextId: project.id, objectiveId: "objective-1", title: id,
...
class MemoryWorkflowRepository {
    state = { id: "workflow-1", status: "running", currentStep: "OBSERVE", checkpointVersion: 0 };
    async getOrCreateWorkflow() { return this.state; }
    async saveSnapshot(_userId, _run, content) {
    ...
    async saveGapAnalysis(_userId, _run, content) {
    ...
    async saveBacklogProposal(_userId, _run, content) {
...
describe("project state projection", () => {
    it("is deterministic and preserves blockers, evidence, and unknown freshness", () => {
        const first = projectProjectState(context());
...
//# sourceMappingURL=project-leadership-service.test.js.map
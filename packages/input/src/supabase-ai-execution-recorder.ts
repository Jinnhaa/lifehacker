  ...
  async record(record: AIExecutionRecord): Promise<void> {
    await this.sql`
      insert into public.ai_executions(
        user_id,agent_run_id,workflow_run_id,job_type,provider,model,prompt_version,input_context_hash,status,
        input_tokens,output_tokens,latency_ms,created_at,completed_at
      ) values (
        ${record.userId},${record.agentRunId ?? null},${record.workflowRunId ?? null},${record.jobType},${record.provider},${record.model},${record.promptVersion},

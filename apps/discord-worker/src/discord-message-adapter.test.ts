...
describe("DiscordMessageAdapter", () => {
  ...
  it("routes a project request through Chief without direct PM mediation or CREATE_TASK", async () => {
    const { processor } = adapterWith();
    const chiefHandler = { handleChiefMessage: vi.fn(async () => ({ handled: true, reply: "LogFolio PM에게 확인했어.\n\n남은 일 2개" })) };
    const adapter = new DiscordMessageAdapter(
      allowedDiscordUserId,
      { resolve: vi.fn(async () => ({ userId, timeZone: "Asia/Seoul" })) },
      processor,
